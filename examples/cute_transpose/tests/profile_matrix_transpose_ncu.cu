...
static std::vector<cudaError_t (*)(float const*, float*, unsigned int, unsigned int, cudaStream_t)> const transpose_functions{
...
TEST_P(TestMatrixTransposeFloat, TestMatrixTransposeFloat)
{
    ...
    {
        std::cout << "NumRepeats: " << NUM_REPEATS << " NumWarmups: " << NUM_WARMUPS << std::endl;
