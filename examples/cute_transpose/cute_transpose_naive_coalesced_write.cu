...
__global__ void
transpose_naive_coalesced_write(TENSOR_SRC tensor_src,
                                       ...
                                       cudaStream_t stream)
{
    ...
    dim3 const thread_dim{cute::size(thread_layout_transposed)};

    transpose_naive_coalesced_write<<<grid_dim, thread_dim, 0, stream>>>(
