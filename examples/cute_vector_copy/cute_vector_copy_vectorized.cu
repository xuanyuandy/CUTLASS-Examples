...
vector_copy_vectorized(TENSOR_SRC tensor_src, TENSOR_DST tensor_dst,
                       unsigned int size, THREAD_LAYOUT, VECTOR_LAYOUT)
{
                              ...
                              unsigned int size, cudaStream_t stream)
{
    ...
    using CopyAtom = cute::Copy_Atom<cute::UniversalCopy<VEC_TYPE>, T>;
    ...
    vector_copy_vectorized<<<grid_dim, thread_dim, 0, stream>>>(
        tiled_tensor_src, tiled_tensor_dst, size, thread_layout, vector_layout);
...
template cudaError_t launch_vector_copy_vectorized<float, int4>(
    ...
    cudaStream_t stream);
template cudaError_t launch_vector_copy_vectorized<double, int4>(
    double const* input_vector, double* output_vector, unsigned int size,
    cudaStream_t stream);