...
static __global__ void general_matrix_multiplication_naive_vectorized(
    ProblemShape shape_MNK, CtaTiler cta_tiler, TA const* A, AStride stride_A,
    ASmemLayout smem_layout_A, AThreadLayout thread_layout_A,
    TiledCopyA tiled_copy_A, TB const* B, BStride stride_B,
    BSmemLayout smem_layout_B, BThreadLayout thread_layout_B,
    TiledCopyB tiled_copy_B, TC* C, CStride stride_C, CSmemLayout,
    CThreadLayout thread_layout_C, Alpha alpha, Beta beta)
{
    ...
    auto thread_copy_A{tiled_copy_A.get_slice(threadIdx.x)};
    ...
    auto thread_copy_B{tiled_copy_B.get_slice(threadIdx.x)};
    ...
    for (auto tile_idx_k{0}; tile_idx_k < num_tiles_k; ++tile_idx_k)
    {
        ...
        cute::copy_if(tiled_copy_A, thread_layout_A_predicate_tensor_A,
                      thread_layout_A_global_block_tensor_A(
                          cute::_, cute::_, cute::_, tile_idx_k),
                      thread_layout_A_smem_tensor_A);
        cute::copy_if(tiled_copy_B, thread_layout_B_predicate_tensor_B,
...
static cudaError_t
gemm_base_new(int m, int n, int k, Alpha alpha, TA const* A, int ldA,
              TB const* B, int ldB, Beta beta, TC* C, int ldC, AStride stride_A,
              BStride stride_B, CStride stride_C, bool use_swizzle_A,
              bool use_swizzle_B, cudaStream_t stream)
{
    // Define GEMM shape.
    auto const M{m};
    auto const N{n};
    ...
    auto const smem_layout_to_use_A{use_swizzle_A ? smem_layout_A_swizzled
                                                  : smem_layout_A};
    auto const smem_layout_to_use_B{use_swizzle_B ? smem_layout_B_swizzled
                                                  : smem_layout_B};

    general_matrix_multiplication_naive<<<grid_dims, block_dims, 0, stream>>>(
        gemm_shape, cta_tiler, A, stride_A, smem_layout_to_use_A,
        thread_layout_A, B, stride_B, smem_layout_to_use_B, thread_layout_B, C,
        stride_C, smem_layout_C, thread_layout_C, alpha, beta);
                           ...
                           int ldC, cudaStream_t stream)
{
    ...
    using VECTOR_TYPE_A = cute::uint128_t;
    constexpr auto NUM_VECTOR_ELEMENTS_A{sizeof(VECTOR_TYPE_A) / sizeof(TA)};
    auto const vector_shape_A{
        cute::make_shape(cute::Int<NUM_VECTOR_ELEMENTS_A>{},
                         cute::Int<1>{})}; // (NUM_VECTOR_ELEMENTS_A, 1)
    auto const vector_stride_A{cute::make_stride(
    ...
    auto const vector_layout_A{cute::make_layout(
        vector_shape_A, vector_stride_A)}; // (NUM_VECTOR_ELEMENTS_A, 1)
    auto copy_A{cute::make_tiled_copy(
        cute::Copy_Atom<cute::UniversalCopy<VECTOR_TYPE_A>, TA>{},
        thread_layout_A,
        vector_layout_A)}; // Thread layout: (THR_M, THR_K) Value layout:
                           // (NUM_VECTOR_ELEMENTS_A, 1)
    using VECTOR_TYPE_B = cute::uint128_t;
    auto const NUM_VECTOR_ELEMENTS_B{sizeof(VECTOR_TYPE_B) / sizeof(TB)};
    auto const vector_shape_B{
        cute::make_shape(cute::Int<NUM_VECTOR_ELEMENTS_B>{},
                         cute::Int<1>{})}; // (NUM_VECTOR_ELEMENTS_B, 1)
    auto const vector_stride_B{cute::make_stride(
    ...
    auto const vector_layout_B{cute::make_layout(
        vector_shape_B, vector_stride_B)}; // (NUM_VECTOR_ELEMENTS_B, 1)
    auto copy_B{cute::make_tiled_copy(
        cute::Copy_Atom<cute::UniversalCopy<VECTOR_TYPE_B>, TB>{},
        thread_layout_B,
        vector_layout_B)}; // Thread layout: (THR_N, THR_K) Value layout:
                           // (NUM_VECTOR_ELEMENTS_B, 1)
                           ...
                           int ldC, cudaStream_t stream)
{
    ...
    using VECTOR_TYPE_A = TA;
    using VECTOR_TYPE_B = TB;
    ...
    general_matrix_multiplication_naive<<<grid_dims, block_dims, 0, stream>>>(
        ...
        thread_layout_A, B, stride_B, smem_layout_B_swizzled, thread_layout_B,
        C, stride_C, smem_layout_C, thread_layout_C, alpha, beta);
