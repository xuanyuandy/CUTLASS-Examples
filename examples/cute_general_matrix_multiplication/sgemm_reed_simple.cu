                            ...
                            int k)
{
    ...
    auto tArA =
        thr_mma.partition_fragment_A(gA(_, _, 0)); // (MMA, MMA_M, MMA_K)
    auto tBrB =
        thr_mma.partition_fragment_B(gB(_, _, 0));      // (MMA, MMA_N, MMA_K)
...
int main()
{
    ...
    std::cout << "MMA atom: " << std::endl;
    cute::print(mma_atom{});
    std::cout << std::endl;
    std::cout << "MMA: " << std::endl;
    cute::print(MMA{});
    std::cout << std::endl;
    std::cout << "Thread size: " << size(MMA{}) << std::endl;
    ...
    for (int i = 0; i < 100; ++i)
    {
        cublasStatus_t ret = cublasHgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, n, m,
                                         k, &alpha, (half*)Bptr, k, (half*)Aptr,
                                         k, &beta, (half*)Cptr_cublas, n);
    ...
    cudaMemcpy(Cptr_cublas_host, Cptr_cublas, sizeof(T) * m * n,
    ...
    Tensor tensor_C =
        make_tensor(Cptr_host, make_shape(m, n), make_stride(n, 1));
    Tensor tensor_C_cublas =
        make_tensor(Cptr_cublas_host, make_shape(m, n), make_stride(n, 1));
