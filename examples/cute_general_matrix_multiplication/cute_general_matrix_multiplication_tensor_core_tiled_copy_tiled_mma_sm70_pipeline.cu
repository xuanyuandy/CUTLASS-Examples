...
//     auto const bP{cute::Int<3>{}}; // Pipeline
...
//     auto const smem_shape_A{cute::make_shape(bM, bK, bP)}; // (BLK_M, BLK_K, BLK_P)
//     auto const smem_stride_A{cute::make_stride(
//         cute::Int<1>{}, bM, bM * bK)}; // column-major
//     auto const smem_layout_A{
//         cute::make_layout(smem_shape_A, smem_stride_A)}; // (BLK_M, BLK_K, BLK_P)
//     auto const smem_shape_B{cute::make_shape(bN, bK, bP)};   // (BLK_N, BLK_K, BLK_P)
//     auto const smem_stride_B{cute::make_stride(
//         cute::Int<1>{}, bN, bN * bK)}; // column-major
//     auto const smem_layout_B{
//         cute::make_layout(smem_shape_B, smem_stride_B)}; // (BLK_N, BLK_K, BLK_P)
...
//     general_matrix_multiplication_tiled_copy_tiled_mma_sm80_pipeline<<<grid_dims, block_dims,
//                                                          0, stream>>>(
